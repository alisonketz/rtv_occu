...
#imputing missing values of covariates set to the mean of each covariate
...
for (s in 1:n_stands) {
  for (y in 1:n_years) {
    for (i in 1:n_plots) {
      y_value <- df_long %>%
        filter(stand == stands[s], year == year[y], plot_id == plots[i]) %>%
        pull(y)
...
num_plots <- matrix(NA, nrow = n_stands,ncol = n_years)
for(s in 1:n_stands){
  for(t in 1:n_years){
    num_plots[s,t] <- length(y_array[s,t,][!is.na(y_array[s,t,])])
  }
}

#centering and scaling the age survival array for the probability of detection
...
### save data structures ready for model fitting, so that I don't have to run these again

save(y_array,file = 'data_product/y_array.Rdata')
save(ht_array,file = 'data_product/ht_array.Rdata')
save(age_surv_array,file = 'data_product/age_surv_array.Rdata')
save(scaled_age_surv_array,file = 'data_product/scaled_age_surv_array.Rdata')
save(age_surv_matrix, file = 'data_product/age_surv_matrix.Rdata')
save(dist_of_t_matrix, file = 'data_product/dist_of_t_matrix.Rdata')
save(dist_of_s_matrix, file = 'data_product/dist_of_s_matrix.Rdata')
save(ht_matrix, file = 'data_product/ht_matrix.Rdata')
save(num_plots, file = 'data_product/num_plots.Rdata')

### load data structures ready for model fitting, so that I don't have to run these again

load('data_product/y_array.Rdata')
load('data_product/ht_array.Rdata')
load('data_product/age_surv_array.Rdata')
load('data_product/scaled_age_surv_array.Rdata')
load('data_product/age_surv_matrix.Rdata')
load('data_product/dist_of_t_matrix.Rdata')
load('data_product/dist_of_s_matrix.Rdata')
load('data_product/ht_matrix.Rdata')

# ###checking on the logprob is NA
# missing <- matrix(0,nr = 1,ncol = 3)
# for (j in 1:n_stands) {
#     for (t in 1:n_years) {
#       for (i in 1:num_plots[j,t]) {
#         if(is.na(y_array[j,t,i])){ cat(paste0(j,"_",t,"_",i,"\n"))
#         missing = rbind(missing,c(j, t, i))
#         k = k+ 1}
#       }
#     }
#   }
